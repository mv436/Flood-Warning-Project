...
def fetch(url):#Fetch data from url and return fetched JSON object
    r = requests.get(url)
    data = r.json()
    return data


def dump(data, filename):#Save JSON object to file
    f = open(filename, 'w')
    data = json.dump(data, f)


def load(filename):#Load JSON object from file
    f = open(filename, 'r')
    data = json.load(f)
    return data
...
def fetch_latest_water_level_data(use_cache=False): #Fetch latest levels from all 'measures'. Returns JSON object
